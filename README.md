...
The tool parses RAM dumps to reconstruct the hierarchy of the standard 3DS memory allocator (**ExpHeap**).
...
* **Pokémon Gen 6 Support**: Specialized handling for XY and ORAS data structures.
...
> To access the latest features and bug fixes, it is highly recommended to **build the project yourself**. The versions
> in the "Releases" tab may not be up to date with the latest source code.

1. Clone the repository: `git clone https://github.com/your-username/CTR-Heap-Mapper.git`
2. Open the project in **CLion** (or your preferred IDE).
3. Ensure your CMake profile is configured to point to your **Qt6** installation.
4. Build & Run.

## ⚠️ Compatibility Notes

* **Pokémon XY / ORAS**: These titles manage memory differently than most 3DS games. They utilize a specific mechanism
  involving a **heap table** that wraps the ExpHeap for resource management. The tool now natively supports this
  structure.
...
* **MemoryBlockHeaders**: Differentiates between `Used` (signature `DU` / `0x5544`) and `Free` (signature `RF` /
  `0x4652`) states.
...
If you encounter any issues or wish to suggest improvements regarding data structures, please open an **Issue** or
contact me directly!